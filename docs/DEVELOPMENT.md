...
| New **tab or main screen** | `MainTabView` + new View + optional ViewModel. Pass `Session` (or specific ViewModels) via `environmentObject`. For **stores** (in-app browser): add a case to the `Store` enum in `AmazonBrowserView.swift`, then add a tab or sidebar link to `StoreBrowserView(store: .yourStore)`. |
