...
| New **tab or main screen** | `MainTabView` + new View + optional ViewModel. Pass `Session` (or specific ViewModels) via `environmentObject`. For **stores** (in-app browser): stores are user-managed and stored in the **Stores** sheet. Add/edit/remove stores in Settings → Stores; the sidebar “Stores” section and in-app browser use `session.stores.stores`. For **sources** (in-app browser): sources are user-managed and stored in the **Sources** sheet; add/edit/remove in Settings → Sources; sidebar "Sources" section and browser use `session.sources.sources`. See [DATA_SCHEMA.md](DATA_SCHEMA.md) for the Stores and Sources sheets. |
| New **API call** (Sheets/Drive) | Implement in `SheetsService` or `DriveService`; call from the appropriate ViewModel. |
| **Export** (e.g. CSV, PDF) | Implement in `ExportService` (data only); trigger from cog menu (macOS) or from the Items tab toolbar (iOS) via `ExportMenuView`. Use `session.inventory.items`, `session.categories.categories`, `session.locations.locations`, and `session.drive` for thumbnails. |
