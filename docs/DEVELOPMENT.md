...
| New **tab or main screen** | `MainTabView` + new View + optional ViewModel. Pass `Session` (or specific ViewModels) via `environmentObject`. Example: Amazon tab uses `AmazonBrowserView` (in-app WKWebView + “Add this item” to open add form with current URL). |
