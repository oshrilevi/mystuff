...
| New **tab or main screen** | `MainTabView` + new View + optional ViewModel. Pass `Session` (or specific ViewModels) via `environmentObject`. For **stores** (in-app browser): stores are user-managed and stored in the **Stores** sheet. Add/edit/remove stores in Settings → Stores; the sidebar “Stores” section and in-app browser use `session.stores.stores`. See [DATA_SCHEMA.md](DATA_SCHEMA.md) for the Stores sheet. |
